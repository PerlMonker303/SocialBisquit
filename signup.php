  ...
  if(empty($email)||empty($firstname)||empty($lastname)||empty($password)||empty($repassword)){
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      $resultCheck = mysqli_stmt_num_rows($stmt);
        if($resultCheck > 0){
          header("Location: ../index.php?error=emailtaken&signupFName=".$firstname."&signupLName=".$lastname);
          exit();
        }else{
          $sql = "INSERT INTO tbl_users(email,fName,lName,password,is_active,is_reported,is_blocked,dateOfReg,updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
          $stmt = mysqli_stmt_init($conn);
          if(!mysqli_stmt_prepare($stmt, $sql)){
            header("Location: ../index.php?error=sqlerror");
            exit();
            ...
            $true = 1;
            $false = 0;
            mysqli_stmt_bind_param($stmt, "ssssiii", $email, $firstname, $lastname, $hashedPassword, $false, $false, $false);
            mysqli_stmt_execute($stmt);
            header("Location: ../index.php?error=sgn_success&$email&$firstname&$lastname&$hashedPassword");
            exit();
          }
        }
    }
